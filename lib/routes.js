/**
 * dsh-remote-ssh-tools — HTTP REST bridge, WebSocket terminal bridge, asset + page
 * serving. All routes hang under the plugin-owned prefix /dsh-remote-ssh-tools and
 ...
 *
 * Wire protocol for /dsh-remote-ssh-tools/ws (JSON text frames):
...
export function pagePathFor(record) {
  return `/dsh-remote-ssh-tools/terminal?id=${encodeURIComponent(record.id)}&token=${encodeURIComponent(record.token)}`;
}
...
export function registerRoutes(ctx, { store, engine, cfg, getState }) {
    ...
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const guard = "/dsh-remote-ssh-tools/api";
          ...
          return sendError(res, "not-found", `no API route for ${method} /dsh-remote-ssh-tools/api/${seg.join("/")}`, 404);
        } catch (error) {
  ...
  disposers.push(
    ctx.webServer.register({
      kind: "exact",
      path: "/dsh-remote-ssh-tools/terminal",
      handler: (req, res) => {
  ...
  disposers.push(
    ctx.webServer.register({
      kind: "prefix",
      path: "/dsh-remote-ssh-tools/assets",
      handler: (req, res) => {
        if (!fence(req, cfg)) { res.writeHead(403); res.end("forbidden"); return; }
        // The DSH webserver hands prefix handlers the FULL path (mount prefix
        // included) — strip our own "/dsh-remote-ssh-tools/assets/" mount first.
        const raw = (req.url ?? "").split("?")[0];
        const prefix = "/dsh-remote-ssh-tools/assets/";
