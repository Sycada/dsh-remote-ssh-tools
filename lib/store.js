/**
 * dsh-remote-ssh-tools — data store (profiles, prefs, host-key cache).
 *
 * Everything is JSON under `<DSH_HOME>/dsh-remote-ssh-tools/store.json`
...
export function storePaths(cfg = {}) {
  const dir = join(dshHomeOf(), "dsh-remote-ssh-tools");
...
export const defaultPasswordRef = (name) => `DSH_REMOTE_SSH_TOOLS_${refSuffixOf(name)}_PASSWORD`;
export const defaultPassphraseRef = (name) => `DSH_REMOTE_SSH_TOOLS_${refSuffixOf(name)}_PASSPHRASE`;
...
function assertRefName(ref, field) {
  if (ref !== undefined && ref !== null && ref !== "" && !REF_NAME_RE.test(ref)) {
    throw new Error(
      `${field} "${ref}" is not a valid credential reference — use only letters, digits and underscores, starting with a letter or underscore (e.g. DSH_REMOTE_SSH_TOOLS_TEST_PASSWORD)`
    );
  }
}
