window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function api(path, options) {
      const res = await fetch("/dsh-remote-ssh-tools/api" + path, { headers: { "content-type": "application/json" }, ...options });
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("settings.plugin.item", () =>
        ctx.slots.register({ name: "settings.plugin.item", id: "dsh-remote-ssh-tools", key: "dsh-remote-ssh-tools", order: 130, inject: () => ({}) }, Card)
      );
      // dedicated full page in the Settings left navigation
      ctx.slots.inject("settings.section", () =>
        ctx.slots.register(
          { name: "settings.section", id: "dsh-remote-ssh-tools", order: 140, label: () => "Remote SSH", inject: () => ({}) },
          SectionCard
        )
      );
    }
